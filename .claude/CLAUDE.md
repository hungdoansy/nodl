...
- **Inline-flex elements in output lines**: Any `display: inline-flex` element inside an aligned output line must use `verticalAlign: 'top'`. Without it, CSS baseline alignment adds ~1px to the parent div's line box (e.g., 22px instead of 21px), breaking alignment with the editor. This applies to ObjectTree's chevron toggle and ConsoleEntry's last-expression arrow (`<- value`). ObjectTree's root is `<span>` when collapsed, `<div>` when expanded.
